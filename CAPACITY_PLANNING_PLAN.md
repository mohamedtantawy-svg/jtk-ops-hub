...
| 3. Team Summary roll-up | ✅ shipped | [#888](https://github.com/Deel-Playground/jtk-ops-hub-v2/pull/888) | `aggregateTeamSummary` derives entirely from the member roll-up (no extra DB hit) + `CapacityTeamSummaryTable` with a dept-wide totals row. Adds an Avg-Total-hrs/day column + Signal pill that the audit doesn't expose — same compute, more glanceable. |
