...
const Sidebar=({user,view,setView,tasks,setTask,escalCount,onSearch,open,setOpen,notifs,markAllRead,onCreateTask,comms})=>{
  ...
  const SectionLabel=({text})=>(
    <div style={{padding:'10px 12px 4px',color:'var(--text-muted)',fontSize:9.5,fontWeight:700,letterSpacing:'.08em',textTransform:'uppercase'}}>{text}</div>
  );
  ...
  if(!open) return(
    <div style={{width:56,background:'var(--surface)',display:'flex',flexDirection:'column',alignItems:'center',padding:'12px 0 8px',flexShrink:0,height:'100%',gap:2}}>
      <div title="Ops Hub" style={{width:32,height:32,background:'var(--g)',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:6,flexShrink:0}}>
        <i className="bi-grid-1x2-fill" style={{color:'var(--text)',fontSize:14}}></i>
      </div>
    ...
    <div style={{width:224,background:'var(--surface)',display:'flex',flexDirection:'column',flexShrink:0,height:'100%'}}>
      ...
      <div style={{padding:'14px 14px 12px',borderBottom:'1px solid #dedede'}}>
        <div style={{display:'flex',alignItems:'center',gap:9}}>
          <div style={{width:30,height:30,background:'var(--g)',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><i className="bi-grid-1x2-fill" style={{color:'var(--text)',fontSize:14}}></i></div>
          <div style={{flex:1}}><div style={{color:'var(--text)',fontWeight:700,fontSize:16,lineHeight:1.2}}>deel.</div><div style={{color:'var(--text-muted)',fontSize:9.5,letterSpacing:'.04em'}}>OPS HUB</div></div>
      ...
      <div style={{padding:'8px 10px 4px',display:'flex',gap:5}}>
        <button aria-label="Search Ops Hub" onClick={onSearch} style={{flex:1,display:'flex',alignItems:'center',gap:8,padding:'7px 11px',borderRadius:8,background:'var(--surface-3)',border:'1px solid #dedede',color:'var(--text-secondary)',fontSize:12.5,cursor:'pointer',textAlign:'left',transition:'all .15s'}}
          ...
          <i className="bi-search" style={{fontSize:12}}></i><span style={{flex:1}}>Search…</span><span style={{background:'#f2f2f2',borderRadius:4,padding:'1px 5px',fontSize:10,fontFamily:'monospace',color:'var(--text-secondary)',border:'1px solid #dedede'}}>⌘K</span>
        </button>
        {perms?.canDo('can_create_task')!==false&&<button onClick={onCreateTask} title="Create Task" style={{width:34,height:34,borderRadius:8,background:'var(--surface)',border:'1px solid #dedede',color:'var(--text)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,transition:'all .15s'}}
      ...
      <div style={{padding:'9px 12px',borderTop:'1px solid rgba(0,0,0,.04)',display:'flex',alignItems:'center',gap:8}}>
        ...
        <div style={{flex:1,minWidth:0}}><div style={{color:'#dedede',fontSize:12.5,fontWeight:600,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{user.name}</div><div style={{color:'var(--text)',fontSize:11}}>{FLAGS[user.country]} {user.team} · {perms?.accessTypeName||'Agent'}</div></div>
