...
const CountryDD=({all,sel,onChange})=>{
  ...
  return(
    <div ref={ref} style={{position:'relative'}}>
      ...
      {open&&<div style={{position:'absolute',top:'calc(100% + 4px)',left:0,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,boxShadow:'0 4px 24px rgba(0,0,0,0.1)',zIndex:100,minWidth:165,padding:'6px 0',animation:'modalIn .12s cubic-bezier(.34,1.56,.64,1) forwards'}}>
        ...
        <div role="listbox" aria-label="Country filter" style={{maxHeight:220,overflowY:'auto'}}>
          {all.map(c=><label key={c} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 14px',cursor:'pointer',fontSize:13,transition:'background .1s'}}
            ...
            <input type="checkbox" checked={sel.includes(c)} onChange={()=>toggle(c)} style={{accentColor:'#1b1b1b',width:16,height:16}}/><span>{getFlag(c)}</span><span style={{color:'var(--text-secondary)',fontWeight:sel.includes(c)?600:400}}>{getCountryName(c) || c}</span>
          </label>)}
        </div>
      </div>}
    </div>
