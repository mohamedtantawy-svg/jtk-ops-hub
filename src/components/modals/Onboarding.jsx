    ...
    <div style={{background:'var(--surface)',borderRadius:16,width:'100%',maxWidth:520,overflow:'hidden',boxShadow:'0 4px 24px rgba(0,0,0,0.15)',position:'relative',animation:'modalIn .18s cubic-bezier(.34,1.56,.64,1) forwards'}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{background:'linear-gradient(135deg,#1b1b1b 0%,#2d2d2d 100%)',padding:'24px 24px 24px'}}>
        <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:16}}>
          <div style={{width:40,height:40,background:'var(--g)',borderRadius:16,display:'flex',alignItems:'center',justifyContent:'center'}}><i className="bi-grid-1x2-fill" style={{color:'var(--text)',fontSize:18}}></i></div>
          <div><div style={{color:'white',fontWeight:700,fontSize:18,lineHeight:1.2}}>Welcome to Ops Hub</div><div style={{color:'var(--text-muted)',fontSize:12.5,marginTop:2}}>HR Operations Command Center · Deel</div></div>
        </div>
        <p style={{color:'var(--text-muted)',fontSize:13,lineHeight:1.65,margin:0}}>Everything your team needs to manage tickets, escalations, deadlines, and team knowledge — unified in one place.</p>
      </div>
      <div style={{padding:'16px 24px 24px'}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:24}}>
          {[
            ...
            <div key={f.title} style={{background:'var(--surface-3)',borderRadius:12,padding:'13px 14px',border:'1px solid var(--border)'}}>
              ...
              <div style={{fontWeight:600,fontSize:13,color:'var(--text)',marginBottom:3}}>{f.title}</div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14}}>
          ...
          <label htmlFor="dont-show" style={{fontSize:12.5,color:'var(--text-secondary)',cursor:'pointer',userSelect:'none'}}>Don't show again on next visit</label>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div style={{fontSize:11.5,color:'var(--text-muted)',textAlign:'right',lineHeight:1.5}}>Press <span style={{background:'#f2f2f2',borderRadius:4,padding:'1px 6px',fontFamily:'monospace',fontSize:11,color:'var(--text)'}}>⌘K</span><br/>to search anywhere</div>
        </div>
      </div>
    </div>
  </div>
