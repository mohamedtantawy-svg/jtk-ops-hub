...
function AvatarStack({ ids, scope, team, max = 4 }) {
  if (scope === 'everyone') return (
    <div style={{ display:'flex', alignItems:'center', gap:5 }}>
      ...
      <span style={{ fontSize:12, color:'var(--text-secondary)' }}>Everyone</span>
    </div>
  );
  if (scope === 'team') return (
    <div style={{ display:'flex', alignItems:'center', gap:5 }}>
      <i className="bi-people" style={{ color:'#1f74b3', fontSize:13 }}/>
      <span style={{ fontSize:12, color:'var(--text-secondary)' }}>{team} Team</span>
    </div>
  );
  if (!ids?.length) return <span style={{ fontSize:12, color:'var(--text-muted)' }}>Unassigned</span>;
    ...
    <div style={{ display:'flex', alignItems:'center' }}>
      ...
      {rest > 0 && (
        <div style={{
          ...
          color:'var(--text-secondary)', fontSize:10, fontWeight:700, display:'flex',
        ...
        <div style={{ padding:'16px 24px 0', flexShrink:0 }}>
          <div style={{ display:'flex', gap:10, marginBottom:20, alignItems:'center', flexWrap:'wrap' }}>
            ...
            <div style={{ position:'relative', flex:1, minWidth:200, maxWidth:320 }}>
              <i className="bi-search" style={{ position:'absolute', left:10, top:'50%', transform:'translateY(-50%)', color:'var(--text-muted)', fontSize:13, pointerEvents:'none' }}/>
              <input
                value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Search projects…"
                style={{
                  width:'100%', padding:'7px 10px 7px 32px', border:'1px solid var(--border)',
                  borderRadius:128, fontSize:13, color:'var(--text)', background:'var(--surface)',
            ...
            <select
              ...
              style={{
                padding:'7px 12px', border:'1px solid var(--border)', borderRadius:128, fontSize:12,
                color:'var(--text-secondary)', background:'var(--surface)', outline:'none', cursor:'pointer', fontFamily:'inherit',
              }}
            >
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding:'14px 16px' }}>
                        ...
                        <div style={{ fontSize:11, color:'var(--text-muted)' }}>{project.id}</div>
                      </td>
                      ...
                      <td style={{ padding:'14px 16px' }}>
                        {(()=>{const ps=PROJECT_STATUS_STYLES[project.status]||{background:'#f2f2f2',color:'var(--text-secondary)'};return(<span style={{padding:'3px 10px',borderRadius:128,fontSize:11,fontWeight:600,whiteSpace:'nowrap',...ps}}>{statusInfo?.label}</span>);})()}
                      </td>
                      ...
                      <td style={{ padding:'14px 16px' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:4, color:'var(--text-secondary)' }}>
                          <i className="bi-chat" style={{ fontSize:12, color:'var(--text-muted)' }}/>
