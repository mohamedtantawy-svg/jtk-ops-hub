    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflowY:'hidden'}}>
      ...
      {slackData?.isAvailable&&(
        <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 14px',borderRadius:10,background:'#f0fdf4',border:'1px solid #bbf7d0',marginBottom:14,fontSize:12}}>
          ...
          <span style={{color:'var(--text-secondary)'}}>— {slackData.channels?.length||0} channels visible{slackData.escalationMessages?`, ${slackData.escalationMessages.length} escalation messages`:''}</span>
          <button onClick={slackData.refresh} style={{marginLeft:'auto',fontSize:11,color:'var(--text-secondary)',background:'var(--surface)',border:'1px solid var(--border)',borderRadius:6,padding:'2px 8px',cursor:'pointer'}}>Refresh</button>
        </div>
      )}

      {/* ── Internal pill tabs ─────────────────────────────────────────── */}
      <div style={{display:'flex',background:'var(--surface-3)',borderRadius:128,padding:3,gap:2,marginBottom:20,width:'fit-content'}}>
        ...
        <>
                ...
                <div key={task.id} style={{background:'var(--surface)',border:isO?'1.5px solid #1f74b3':'1px solid var(--border-light)',borderRadius:'var(--radius-xl)',boxShadow:'0 1px 2px rgba(0,0,0,0.04)',transition:'box-shadow .15s, border-color .15s',overflow:'hidden',marginBottom:12,borderBottom:'1px solid var(--border-light)'}}
                  ...
                  <div style={{padding:'14px 18px 12px',display:'flex',alignItems:'flex-start',gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:4,flexWrap:'wrap'}}>
                        <span style={{fontWeight:700,color:'var(--text)',fontSize:14}}>{task.sender}</span>
                        ...
                        <span style={{color:'var(--text-muted)',fontSize:12}}>· {rel(task.minutesAgo)}</span>
                        <FnBadge type={task.type}/>
                      </div>
                      <div style={{color:'var(--text-secondary)',fontSize:13,lineHeight:1.65}}>{task.body}</div>
                    </div>
                    <a href={getUrl(task)} target="_blank" rel="noreferrer" style={{color:'var(--text-muted)',fontSize:13,textDecoration:'none',padding:4,borderRadius:8}}><i className="bi-box-arrow-up-right"></i></a>
                  </div>
                  ...
                  {isO&&!isSent&&<div className="fade-in" style={{margin:'0 18px 14px'}}>
                    ...
                    <div style={{padding:'8px 0 0',display:'flex',gap:7}}>
                      ...
                      <button onClick={()=>setOpenR(null)} style={{height:36,padding:'0 14px',borderRadius:128,border:'1px solid var(--border)',background:'var(--surface)',color:'var(--text-secondary)',fontSize:13,cursor:'pointer',fontWeight:500}}>Cancel</button>
                    </div>
                  </div>}
          ...
          {done.length>0&&(
            <div style={{marginTop:24}}>
              <div style={{fontSize:13,fontWeight:600,color:'var(--text-muted)',letterSpacing:'normal',textTransform:'none',marginBottom:10}}>Addressed ({done.length})</div>
              {done.map(task=>(
                <div key={task.id} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,padding:'12px 16px',marginBottom:8,display:'flex',alignItems:'center',gap:10,opacity:.45}}>
                  <i className="bi-check-circle-fill" style={{color:'#29811e',fontSize:15,flexShrink:0}}></i>
                  <div style={{flex:1,minWidth:0}}><div style={{fontSize:13,color:'var(--text-secondary)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{task.subject}</div><div style={{fontSize:11,color:'var(--text-muted)'}}>{task.sender} · {task.channel} · {rel(task.minutesAgo)}</div></div>
                </div>
              ))}
            </div>
          )}
        ...
        <div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            {LITIGATION_CHANNELS.map(lit=>{
              ...
              return(
                <div key={lit.id} style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,boxShadow:'0 1px 2px rgba(0,0,0,0.04)',overflow:'hidden',transition:'box-shadow .15s'}}
                  ...
                  <div style={{padding:'14px 18px',display:'flex',alignItems:'flex-start',gap:12}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6,flexWrap:'wrap'}}>
                        ...
                        <span style={{fontSize:13,fontWeight:500,color:'var(--text-secondary)'}}>{getFlag(lit.country)||'🌐'} {getCountryName(lit.country) || lit.country}</span>
                      ...
                      {/* Employee + issue */}
                      <div style={{fontSize:14,fontWeight:600,color:'var(--text)',marginBottom:2}}>{lit.employeeName}</div>
                      <div style={{fontSize:13,color:'var(--text-secondary)',marginBottom:8}}>{lit.issue}</div>
                      {/* Meta row */}
