...
const STATUS_CONFIG = {
  ...
  warning:  { icon: 'bi-exclamation-circle-fill',   border: '#ffe27c' },
  active:   { icon: 'bi-arrow-repeat',              border: '#bddcf0' },
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: '#fafaf9' }}>
      ...
      <div style={{ padding: '12px 24px', background: 'white', borderBottom: '1px solid #f0efed', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 11, color: '#9e9e9e' }}>{totalFiltered} {totalFiltered === 1 ? 'case' : 'cases'}</span>
      </div>
      ...
      {loading && byCountry.length === 0 && (
        <div style={{ textAlign: 'center', padding: 60 }}>
          ...
          <div style={{ fontSize: 14, color: '#9e9e9e' }}>Loading offboarding cases...</div>
        </div>
      )}
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: 60 }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 600, color: '#1b1b1b', marginBottom: 6 }}>
            {searchTerm || statusFilter ? 'No matches' : 'No active offboarding cases'}
          </div>
          <div style={{ fontSize: 13, color: '#9e9e9e' }}>
            {searchTerm || statusFilter ? 'Try adjusting the filters' : 'All termination cases have been resolved'}
          </div>
        </div>
      )}
        ...
        return (
          <div key={group.country} style={{ borderBottom: '1px solid #f0efed' }}>
            ...
            >
              ...
              <span style={{ fontSize: 12, color: '#9e9e9e' }}>{group.people.length} {group.people.length === 1 ? 'case' : 'cases'}</span>
              {group.urgentCount > 0 && (
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 8px', borderRadius: 128, background: '#fef2f2', color: '#d42d35', fontSize: 10, fontWeight: 700 }}>
                  <i className="bi-exclamation-triangle-fill" style={{ fontSize: 9 }} />{group.urgentCount} urgent
                </span>
              )}
...
function PersonRow({ person }) {
  ...
  const sev = person.status?.severity || 'info';
  const cfg = STATUS_CONFIG[sev] || STATUS_CONFIG.info;
  ...
  const daysLeft = person.daysUntilEnd;
  let daysDisplay = '--';
  let daysColor = '#9e9e9e';
  if (daysLeft !== null && daysLeft !== undefined) {
    if (daysLeft < 0) {
      daysDisplay = `${Math.abs(daysLeft)}d overdue`;
      daysColor = '#d42d35';
    } else if (daysLeft === 0) {
      daysDisplay = 'Today';
      daysColor = '#d42d35';
      ...
      daysColor = daysLeft <= 14 ? '#d42d35' : daysLeft <= 30 ? '#ed8d00' : '#616161';
    }
  }
    ...
    >
      ...
      <td style={{ ...tdStyle, textAlign: 'left', paddingLeft: 36 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{
            width: 28, height: 28, borderRadius: '50%', background: '#fce8e8',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 10, fontWeight: 700, color: '#d42d35', flexShrink: 0,
          }}>
      ...
      <td style={tdStyle}>
        <span style={{
          ...
          background: `${person.status?.color || '#9e9e9e'}12`,
          color: person.status?.color || '#9e9e9e',
      ...
      {/* Days left */}
      <td style={{ ...tdStyle, fontSize: 12, fontWeight: 600, color: daysColor, whiteSpace: 'nowrap' }}>
        {daysDisplay}
      </td>
      ...
      {/* Requested date */}
      <td style={{ ...tdStyle, fontSize: 12, color: '#9e9e9e', whiteSpace: 'nowrap' }}>
        {requestedDate}
      </td>
