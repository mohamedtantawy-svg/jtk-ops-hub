// ── useActiveEorData hook ──────────────────────────────────────────────────
// 2026-06-09: HRX-only data source backing the "Active EOR" queue (the
// post-onboarding Active.*.AwaitingReview review tasks). Mirrors
// useImmigrationCasesData / useOnboardingData (SWR, IDB cache, BroadcastChannel
// sync, per-dept-namespaced cache, 3-strike error debounce, auto-refresh while
// visible) so downstream layers — SourceTable rendering, sync-badge tracking,
// home counts — read it through the same idioms as every other Deel queue.
// Non-HRX depts get `disabled: true` from the route + empty cache, so this hook
// is a no-op outside HRX scope.
import { useState, useEffect, useCallback, useRef } from 'react';
import { fetchDeelActiveEor } from '../services/integrationsApi';
...
const SOURCE_ID = 'active_eor';
const CACHE_TTL = 5 * 60 * 1000;   // matches the route's 5-min fresh window
const CACHE_KEY_BASE = 'ops_hub_active_eor_cache';
// Per-dept namespace so a non-HRX viewer (empty cache) never sees HRX rows from
// a stale cache after a dept switch.
...
export function useActiveEorData(enabled = true, userEmail = null) {
  ...
  const refresh = useCallback(async (force = false) => {
    ...
    const run = (async () => {
      try {
        const res = await fetchDeelActiveEor({ bustCache: force });
        ...
        // Disabled / dept-not-supported: server hard-says zero for this scope.
        // Drop stale rows so the FE renders empty, never leaks another dept.
        if (res?.disabled) {
          setItems([]);
        ...
        console.warn('[useActiveEorData] Failed:', err.message, `(streak ${failStreakRef.current})`);
  ...
  return { items, loading, isRefreshing, error, lastSyncAt, refresh };
}
